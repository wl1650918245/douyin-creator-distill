...
| 项目 | 状态 | 证据 / 边界 |
| --- | --- | --- |
| 轻量模式 | 已通过国内源实测安装 | ModelScope `AI-ModelScope/bge-small-zh-v1.5`；核心权重 `95,827,648` 字节，支持检测、下载、删除和切换。 |
| 高精度模式 | 已通过国内源实测安装 | ModelScope `Qwen/Qwen3-Embedding-0.6B`；核心权重 `1,191,586,416` 字节，与轻量模型独立保存，可自由切换。 |
| 本地存储 | 已实现 | 权重只写入 `runtime/models/embedding/`，目录被 Git 忽略，不引用其他项目路径。 |
| 精排复核 | 已确认并接入配置状态 | 复用 `config/model.config.json` 中的当前分析模型；后端只向页面返回模型名和配置状态，不返回密钥。 |
| 语义召回工作流 | 已实现并完成真实目录回归 | 内容工作台可显式建立或更新 SQLite 向量索引，本地搜索会联动作品牌目录并显示相关度；精排复核仅在用户点击后调用分析模型。2026-08-04 使用姜胡说最新目录实测索引 645 条作品，按来源资产账本复用历史转写正文并形成 3,419 个语义片段，重复更新复用 645 条、重算 0 条；“普通人如何赚钱”与“Skill 知识蒸馏”均召回语义相关作品。 |
| 跨来源智能检索 | 已实现并完成真实资产回归 | 独立页面可搜索全部或指定的已索引关注博主、收藏夹来源；结果保留来源和证据片段。2026-08-06 本机读取 2 个来源、726 条作品、3,520 个片段，真实 API 与 Playwright 查询通过。 |
...
4. 增加图文 OCR 第三通道：重新获取图文图片地址，逐图 OCR 后按作品落盘；当前 Whisper 不能处理图文，云端链接解析失败时只保留标题、简介和互动数据。
...
| 2026-08-01 | 补齐转写对象和关注资产入口。 | 工作台实时显示当前作品标题、ID 和实际通道；关注页三个统计卡分别进入作品目录、定位抓取版本、打开该来源的云端/Whisper 本地资产。江湖说批次安全暂停于 367 条，服务升级至 `2026-08-01.2` 后从原断点恢复，现场显示作品 `7351774759887080719` 正由本地 Whisper 处理。 |
| 2026-08-02 | 建立智能筛选模型管理底座。 | 设置中心可管理轻量与高精度本地模型，支持检测、按需下载、删除和切换；精排复核复用当前分析模型。隔离单元测试覆盖切换、安装检测、删除和密钥不外泄。 |
| 2026-08-04 | 完成智能筛选本地语义索引与作品目录联动。 | 轻量模型实测输出 512 维向量，高精度模型实测输出 1024 维向量；姜胡说 645 条目录结合历史转写正文形成 3,419 个片段，第二次更新全部复用；API 查询和 Playwright 页面搜索均通过。 |
| 2026-08-06 | 增加独立智能检索页、失败转写续接选择和设置分组。 | 跨来源真实索引为 2 个来源、726 条作品、3,520 个片段；失败作品可查看逐条原因后重新选择云端优先或本地优先，只重排失败项；设置中心拆为“系统与账号”和“模型与 API”。单元、API 和 Playwright 回归通过。 |
| 2026-08-06 | 阻止图文作品在云端与 Whisper 之间无效循环。 | 姜胡说剩余 7 条经真实 JSON 核对均为 `awemeType=68` 图文，每条云端累计失败 18 次；接口现返回 `contentType=image` 和 `requiresOcr=true`，页面禁用两个无效重试选项并保留元数据，静态检查和真实页面回归通过。 |
| 2026-08-06 | 将转写错误分类与重试门禁下沉到服务端。 | SQLite 新增 `error_class`、`retryable`、`terminal_reason`；只有限流和网络瞬时错误允许重试，未知错误默认停止，总尝试上限为 6 次。真实姜胡说任务的 7 条图文已回填为确定性失败，重试接口返回 400 且尝试次数保持 18；路由、批处理和静态检查通过。 |
| 2026-08-02 | 国内镜像完成双模型实装。 | 通过 ModelScope 国内源下载并逐文件校验官方大小；轻量权重 `95,827,648` 字节，高精度权重 `1,191,586,416` 字节，两个目录均无未完成文件，默认启用轻量模式；`check`、模型管理测试和 `doctor` 均通过。 |
| 2026-08-02 | 修复收藏夹登录窗口长期占用专用 Chrome Profile。 | 定位到登录助手在验证成功后额外停留 10 分钟；现改为等待 2 秒写盘后自动关闭，并记录登录助手 PID 防止重复启动。真实独立账号回归在约 7 秒内读取 35 个收藏夹并进入等待选择；登录助手实测约 13.5 秒自动退出且 Profile 已释放，静态检查和完整单元回归通过。 |
