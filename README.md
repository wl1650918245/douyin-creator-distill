...
| 智能检索 | 用自然语言跨关注博主、收藏夹和已索引知识资产召回相关作品 | 本地召回不消耗 API；只有用户点击“精排复核”才调用当前分析模型 |
...
传统关键词搜索只能找到标题里出现过的词。CreatorDistill 的智能检索会同时理解作品标题、作者简介和已经完成的转写正文，例如输入“普通人如何赚钱”“如何搭建个人知识体系”，即可召回表达不同、但主题相近的作品。除当前目录内筛选外，独立的“智能检索”页面还可以同时搜索多个关注博主、收藏夹和其他已建立索引的作品资产。

```text
自然语言问题
  -> 本地 Embedding 语义召回
  -> 标题 + 简介 + 转写正文证据片段
  -> 当前目录联动，或在智能检索页跨来源展示作品和相关度
  -> 可选：点击“精排复核”，再由分析模型重排前 20 条
```

- **正文级搜索**：不仅搜标题，还会索引云端转写和本地 Whisper 正文；同一作品优先使用 Whisper 材料。
- **跨来源检索**：可搜索全部已索引资产，也可只勾选特定关注博主或收藏夹；每条结果都显示来源和命中证据。
...
真实回归：姜胡说 645 条作品结合历史转写正文形成 **3,419 个语义片段**；第二次更新复用 **645 / 645** 条作品，重新计算 **0** 条。跨来源页面已在本机同时读取姜胡说和飞天闪客共 **726 条作品、3,520 个片段**并返回带来源的真实结果。详细契约与边界见[智能筛选与语义索引](docs/智能筛选与语义索引.md)。
...
| 云端/Whisper 优先级续接 | 已实现 | 结构化错误分类、失败隔离、6 次总上限和图文 OCR 边界实测 |
| 爆款拆解 | 已实现 | 多作品报告落盘 |
| 选题顾问 | 已实现 | 浏览器 UI 回归 |
| 博主智能体与审稿 | 已实现 | 浏览器 UI 回归 + 审计门禁 |
| 跨资产智能检索 | 已实现 | 2 个真实来源、726 条作品、3,520 个片段 + Playwright 页面回归 |
...
- [2026-08-05 至 2026-08-06 迭代复盘](docs/2026-08-05-至-2026-08-06-迭代复盘.md)
