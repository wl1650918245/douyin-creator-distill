...
for (const id of ["search-view", "global-source-list", "global-all-assets", "global-semantic-query", "global-semantic-submit", "global-semantic-rerank", "global-search-results"]) {
  assert(html.includes(`id="${id}"`), `缺少跨资产智能检索控件 ${id}`);
}
...
for (const endpoint of ["/api/semantic-sources", "/api/semantic-search/global"]) {
  assert(server.includes(endpoint), `缺少跨资产智能检索接口 ${endpoint}`);
}
...
assert(app.includes("runGlobalSemanticSearch(false)"), "跨资产搜索按钮未连接本地召回");
assert(app.includes("runGlobalSemanticSearch(true)"), "跨资产精排按钮未连接显式精排");
assert(app.includes("result.sourceName"), "跨资产结果未显示来源名称");
